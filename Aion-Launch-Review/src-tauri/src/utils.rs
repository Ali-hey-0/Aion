...
impl CodexProcessProvider {
    ...
    fn launch_isolated_codex_process(
        ...
        let path_str = absolute_sandbox_path.to_string_lossy();
        let clean_sandbox_path = path_str.strip_prefix(r"\\?\").unwrap_or(path_str.as_ref());
        let executable_str = executable.to_string_lossy();
        let clean_executable_path = executable_str
            .strip_prefix(r"\\?\")
            .unwrap_or(executable_str.as_ref());

        prepare_codex_home_for_launch(absolute_sandbox_path)?;

        let bat_path = external_launcher_path(absolute_sandbox_path)?;
        let clean_bat_path = normal_windows_path_text_from_str(&bat_path.to_string_lossy());

        let bat_content = format!(
            "@echo off\r\nset \"CODEX_HOME={}\"\r\nstart \"\" \"{}\" --user-data-dir=\"%CODEX_HOME%\"\r\nexit /b 0\r\n",
            clean_sandbox_path, clean_executable_path
        );

        let mut file = File::create(&bat_path).map_err(|error| {
            format!(
                "Failed to create Codex launch script '{}': {error}",
                bat_path.display()
            )
        })?;
        file.write_all(bat_content.as_bytes()).map_err(|error| {
            format!(
                "Failed to write Codex launch script '{}': {error}",
                bat_path.display()
            )
        })?;
        file.flush().map_err(|error| {
            format!(
                "Failed to flush Codex launch script '{}': {error}",
                bat_path.display()
            )
        })?;
        drop(file);

        let mut command = Command::new(system_cmd_executable());
        command.arg("/d").arg("/c").arg(&clean_bat_path);
        scrub_launcher_environment(&mut command);

        let child = command.spawn().map_err(|error| {
            format!(
                "Failed to execute Codex launch script '{}' for sandbox '{}': {error}",
                bat_path.display(),
                clean_sandbox_path
            )
        })?;
        ...
        wait_for_primary_codex_process_using_user_data_dir(absolute_sandbox_path)?.ok_or_else(
            || {
                format!(
                    "Codex launch script completed, but no Codex.exe process appeared with the isolated profile path '{}'. Close any unmanaged/global Codex windows and launch this profile again.",
                    clean_sandbox_path
                )
            },
        )
...
#[cfg(windows)]
fn scrub_launcher_environment(command: &mut std::process::Command) {
    const PRESERVED_ENVIRONMENT_KEYS: &[&str] = &[
        "ALLUSERSPROFILE",
        "APPDATA",
        "COMMONPROGRAMFILES",
    ...
    for key in PRESERVED_ENVIRONMENT_KEYS {
    ...
    let clean_path_segments = std::env::split_paths(&path_value)
        .filter(|segment| !is_development_path_segment(segment))
        .collect::<Vec<_>>();

    if let Ok(clean_path) = std::env::join_paths(clean_path_segments) {
        command.env("PATH", clean_path);
    }
}

#[cfg(windows)]
fn system_cmd_executable() -> PathBuf {
...
#[cfg(windows)]
fn is_development_path_segment(path: &Path) -> bool {
    let text = path
        .to_string_lossy()
        .replace('/', "\\")
        .to_ascii_lowercase();
    text.contains("\\node_modules\\.bin")
        ...
        || text.contains("\\src-tauri\\target\\debug")
        || text.contains("\\target\\debug\\build\\")
        || text.contains("\\target\\debug\\deps")
...
#[cfg(windows)]
fn external_launcher_path(absolute_sandbox_path: &Path) -> Result<PathBuf, String> {
    let launchers_dir = std::env::temp_dir().join("Aion").join("Launchers");
    fs::create_dir_all(&launchers_dir).map_err(|error| {
        format!(
            "Failed to create Aion launch script directory '{}': {error}",
            launchers_dir.display()
        )
    })?;
    ensure_directory_is_not_reparse_point(&launchers_dir, "Aion launch script directory")?;

    let profile_key = absolute_sandbox_path
        .file_name()
        ...
        .unwrap_or_else(|| format!("profile-{}", std::process::id()));

    Ok(launchers_dir.join(format!("aion-launch-{profile_key}.cmd")))
}
